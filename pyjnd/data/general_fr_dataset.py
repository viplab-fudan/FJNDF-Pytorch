...
class GeneralFRDataset(BaseJNDDataset):
    ...
    def init_path_jnd(self, opt):
        ...
        for row in self.meta_info.values:
            ...
            self.paths_jnd.append([input_path, ref_path, yuv_w, yuv_h, yuv_fmt, yuv_bd])
    ...
    def __getitem__(self, index):
        ...
        if ext == '.yuv':
            w       = self.paths_jnd[index][2]
            h       = self.paths_jnd[index][3]
            fmt     = self.paths_jnd[index][4]
            bd      = self.paths_jnd[index][5]
        ...
        return {'img': img_tensor, 'ref_img': ref_tensor, 'img_path': input_path, 'ref_path': ref_path}