...
# ========================================================================================
# Internal Network Implementations (from lle.py)
# ========================================================================================

class _MobileIELLENet_Slim(nn.Module):
    """Internal SLIM network for INFERENCE."""
    def __init__(self, in_channels, out_channels, base_channels):
        super(_MobileIELLENet_Slim, self).__init__()
        self.head = FSTS(
            nn.Sequential(
...
class _MobileIELLENet_Train(nn.Module):
    """Internal complex network for TRAINING."""
    def __init__(self, in_channels, out_channels, base_channels, rep_scale):
        super(_MobileIELLENet_Train, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        ...
        self.head = FST(
            nn.Sequential(
                MBRConv5(in_channels, base_channels, rep_scale=rep_scale),
                nn.PReLU(base_channels),
                MBRConv3(base_channels, base_channels, rep_scale=rep_scale)
            ),
            base_channels
        )
        self.body = FST(
        ...
        self.att = nn.Sequential( 
            nn.AdaptiveAvgPool2d(1),
            MBRConv1(base_channels, base_channels, rep_scale=rep_scale),
            nn.Sigmoid()
        )
        self.att1 = nn.Sequential( 
            MBRConv1(1, base_channels, rep_scale=rep_scale),
            nn.Sigmoid()
        )
        self.tail = MBRConv3(base_channels, out_channels, rep_scale=rep_scale)
    ...
    def slim(self):
        ...
        net_slim = _MobileIELLENet_Slim(self.in_channels, self.out_channels, self.base_channels)
...
class MobileIENet(nn.Module):
    ...
    def __init__(self,
        ...
        # Placeholder for the inference-time network
